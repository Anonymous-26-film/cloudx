...
export function MovieHubPage() {
  ...
  // Sidebar state — default from localStorage, fallback true (open)
  ...
  return (
    <>
      ...
      <div className="min-h-screen pt-16">
        ...
        >
          {/* Top bar */}
          <div className="sticky top-16 z-20 bg-background/95 backdrop-blur-sm border-b border-border/50 px-4 md:px-6 py-3">
            ...
            {!selectedServer && !playingMovieId && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                <div className="text-center py-12 mb-6 bg-secondary/20 rounded-2xl border border-border/30">
                  ...
                  <button onClick={toggleSidebar} className="mt-4 inline-flex items-center gap-2 bg-primary hover:bg-primary/90 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-colors">
            ...
            {playingMovieId > 0 && !showPreview && (
              <motion.div key={iframeKey} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }}>
                <div className="flex items-center gap-3 mb-3">
                  <button
                    onClick={toggleSidebar}
                    className="px-3 py-1.5 text-xs rounded-lg bg-secondary text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1.5"
                  >
                    <Server className="w-3.5 h-3.5" /> {sidebarOpen ? "Close Servers" : "Change Server"}
                  </button>
                  {selectedServer ? (
